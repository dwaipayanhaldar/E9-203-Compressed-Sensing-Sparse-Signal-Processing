...
\begin{frame}
\vfill
\centering{\Huge $\ell_1$-K-SVD}\vfill
\end{frame}

\begin{frame}{$\ell_1$-K-SVD - Basic Formulation}
    \[
...
\begin{frame}{$\ell_1$-K-SVD - Sparse Coding Step}
    \[
...
\begin{frame}{$\ell_1$-K-SVD - Dictionary Update Step}
    \[
    \min_{D,X} \|Y-DX\|_1
    \qquad
    \text{s.t. } \|d_j\|_2 = 1 \text{ for all } j,\;
...
\begin{frame}{$\ell_1$-K-SVD - Dictionary Update Step}
    That $\ell_1$ norm can be approximated by reweighted $l_2$ metric 
    \[
...
\begin{frame}{$\ell_1$-K-SVD - Final Algorithm to find the best u and v}
\scriptsize
\begin{itemize}
...
\begin{frame}{$\ell_1$-K-SVD - Remarks}
\begin{itemize}
    \item After each sparse coding step, any coefficient satisfying
    \[
    |x_i(j)| < T_0 \cdot \|X\|_F
    \]
...
\begin{itemize}
    ...
    \]
    is pruned to zero. The paper uses $T_0 = 0.03$.
    \item The dictionary update is the bottleneck:
    $K$ atoms $\times$ $J$ inner IRLS iterations, each an $m\times m$ linear solve.
    $\;\Rightarrow\;\mathcal{O}(K\cdot J\cdot m^3)$ work per outer iteration —
    significantly heavier than the single SVD per atom in K-SVD.
\end{itemize}
...
\begin{itemize}
    \item $y = Dx + w$, where $w$ is noise
    (Gaussian, Laplacian, or missing pixels).
    \item[] \phantom{Train on patches so that $D$
    captures dominant patterns — edges, textures, colour gradients.}
    \item[] \phantom{$\hat{y} = D\hat{x}$ — sparse $\hat{x}$ discards the noise component.}
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item $y = Dx + w$, where $w$ is noise
    (Gaussian, Laplacian, or missing pixels).
    \item Train on patches so that $D$
    captures dominant patterns — edges, textures, colour gradients.
    \item[] \phantom{$\hat{y} = D\hat{x}$ — sparse $\hat{x}$ discards the noise component.}
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Train on patches so that $D$
    captures dominant patterns — edges, textures, colour gradients.
    \item $\hat{y} = D\hat{x}$ — sparse $\hat{x}$ discards the noise component.
\end{itemize}
\end{frame}


\begin{frame}{Paper Result}
\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{Output_Images/Fig6.jpg}
    \caption{Comparison of denoising performance of the K-SVD and
    $\ell_1$-K-SVD algorithms for Laplacian noise:
    (a)~ground truth (clean) image;
    (b)~noisy input, PSNR\,=\,22.16\,dB, SSIM\,=\,0.36;
    (c)~denoised image using K-SVD, PSNR\,=\,29.78\,dB, SSIM\,=\,0.73;
    (d)~denoised image using $\ell_1$-K-SVD, PSNR\,=\,30.34\,dB, SSIM\,=\,0.81.
    The values of $\lambda$ and $n_p$ chosen in the experiment are 1 and 0.05,
    respectively.}
\end{figure}
\end{frame}

\begin{frame}{Simulation Result A — Gaussian Noise}
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{Output_Images/gaussian_noise.png}
    \caption{Left to right: clean image, noisy input ($\sigma=20/255$),
...
\begin{frame}{Simulation Result B — Laplacian Noise}
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{Output_Images/laplacian_noise.png}
    \caption{Same test image corrupted with Laplacian noise.
    $\ell_1$-K-SVD achieves higher PSNR and SSIM because its
    $\ell_1$ optimization term is the natural MAP estimator under a
    Laplacian noise model, while K-SVD's $\ell_2$ criterion is
    sensitive to heavy tails.}
\end{figure}
\end{frame}

\begin{frame}{Simulation Result C — Inpainting}
\begin{figure}
    \centering
...
\begin{enumerate}
    ...
    \item S. Mukherjee, R. Basu, and C.~S. Seelamantula,
    ``$\ell_1$-K-SVD: A robust dictionary learning algorithm with simultaneous
    update,''
    \textit{Signal Process.},
    vol.~123, pp.~42--52, Jun.\ 2016.
\end{enumerate}
\end{frame}
...
\begin{frame}
\vfill
\centering{\Huge Thank you!}\vfill
\end{frame}
...
\min_{D,\,\mathbf{x}_n}\;\sum_{n=1}^{N}\|\mathbf{y}_n-D\mathbf{x}_n\|_p^p+\lambda_n\|\mathbf{x}_n\|_p^p
\]
comes from the generalised p-gaussian distribution 
\[
...
\]
\textbf{Modified dictionary update weight}:
\[
